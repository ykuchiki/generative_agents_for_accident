...
def get_random_alphanumeric(i=6, j=6):
    """
    指定された範囲内の長さを持つランダムな英数字文字列を返す
    :param i: 文字列の最小値
    :param j: 文字列の最大値
    :return: iからjの範囲の長さを持つランダムな英数字文字列
    """
    k = random.randint(i, j)
    # string.ascii_lettersは英大文字と小文字
    # string.digitsは0から9までの数字
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
    """
    指定されたペルソナが何時に起きるかを示す整数値を返す．モデルはgpt-3.5-turbo-instruct
    :param persona: ペルソナクラスのインスタンス
    :param test_input: テスト用の入力(オプション)
    :param verbose: 詳細情報を表示するかどうか(オプション)
    :return: ペルソナが起きる時間を示す整数
    """
    def create_prompt_input(persona, test_input=None):
        """テスト入力があればそれを使用，なければペルソナの情報からプロンプトを生成"""
        if test_input: return test_input
        # このプロンプト同じissとそれ以外で情報にダブりある気がするけど
    ...
    def __func_clean_up(gpt_response, prompt=""):
        """GPTのレスポンスを処理して起床時間を整数として返す"""
        cr = int(gpt_response.strip().lower().split("am")[0])  # 空文字削除->小文字化->amで分割して数字の部分を抽出->整数型
        return cr
    ...
    prompt_template = "backend/persona/prompt_template/v2/wake_up_hour_v1.txt"
...
def run_gpt_prompt_daily_plan(persona, wake_up_hour, test_input=None, verbose=False):
    """
    指定されたペルソナの1日の計画を生成する．モデルはgpt-3.5-turbo-instruct
    :param persona: ペルソナクラスのインスタンス
    :param wake_up_hour: ペルソナが起きる時間を示す整数(例：6)
    :param test_input: テスト用の入力(オプション)
    :param verbose: 詳細情報を表示するかどうか(オプション)
    :return: 1日の計画
    例： 'wake up and complete the morning routine at 6:00 am',
        'eat breakfast at 7:00 am',..
    """
    def create_prompt_input(persona, wake_up_hour, test_input=None):
        """wake_up_hourなどからプロンプト文を生成する"""
        if test_input: return test_input
        prompt_input = []
        prompt_input += [persona.scratch.get_str_iss()]
    ...
    def __func_clean_up(gpt_response, prompt=""):
        cr = []
        _cr = gpt_response.split(")")  # 応答を")"で分割
        for i in _cr:  # 分割したぶん回す
            # フォーマットとして")"の一個前やリストの各要素の末尾が数字であることが期待されてるみたい
            if i[-1].isdigit():  # 各要素の最後の文字が数字であるかを確認
                i = i[:-1].strip()  # もし末尾が数字ならそれを取り除く
                if i[-1] == "." or i[-1] == ",":  # ピリオドやカンマも取り除く
                    cr += [i[:-1].strip()]  # それらを改めて結合する
        return cr
    ...
    prompt_template = "backend/persona/prompt_template/v2/daily_planning_v6.txt"
...
def run_gpt_prompt_generate_hourly_schedule(persona, curr_hour_str, p_f_ds_hourly_org, hour_str, intermission2=None,
                                            test_input=None, verbose=False):
    """ペルソナの現在の時間に基づいて，時間ごとのスケジュールを生成する．モデルは"gpt-3.5-turbo-instruct"""
    def create_prompt_input(persona, curr_hour_str, p_f_ds_hourly_org, hour_str, intermission2=None, test_input=None):
        """スケジュールフォーマットを [<現在の日付> -- <時間>] Activity: [Fill in] と指定"""
        if test_input: return test_input
        schedule_format = ""
        for i in hour_str:
        ...
        # ペルソナの1日のスケージュールを説明する用
        # Here the originally intended hourly breakdown of [FirstName]'s schedule today: 1) [アクティビティ1], 2) [アクティビティ2], 3) [アクティビティ3]
        ...
        if p_f_ds_hourly_org:
            prior_schedule = "\n"
            # これまでの各スケジュールには一意のIDが付与される，その日の日時と活動内容の文字列も追加
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50,
                 ...
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
    prompt_template = "backend/persona/prompt_template/v2/generate_hourly_schedule_v2.txt"
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    """
    与えられたタスクを指定された期間に分解するための関数．ペルソナの現在のスケジュールを元にタスク分解して，その結果を返す．
    """
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 1000,
                 ...
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/task_decomp_v3.txt"
...
def run_gpt_prompt_action_sector(action_description,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v1/action_location_sector_v1.txt"
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
    """アクションゲームオブジェクトとその説明を元にアクションを生成する関数"""
    ...
    def __func_clean_up(gpt_response, promppt=""):
        cr = int(gpt_response.strip)
        if cr[-1] == ".": cr = cr[:-1]
        return cr
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_obj_event_v1.txt"  ########
...
def run_gpt_prompt_action_arena(action_description,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v1/action_location_object_vMar11.txt"
...
def run_gpt_prompt_action_game_object(action_description,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v1/action_object_v2.txt"
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_pronunciatio_v1.txt"  ########
    prompt_input = create_prompt_input(action_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "🛁🧖‍♀️"  ########
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False):
    """アクションの説明を元にイベントトリプル(人物，動作，対象)を生成する関数"""
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 30,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
    prompt_template = "backend/persona/prompt_template/v2/generate_event_triple_v1.txt"
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
    # prompt_template = "backend/persona/prompt_template/v2/generate_obj_event_v1.txt"
    # prompt_input = create_prompt_input(act_game_object, act_desp, persona)
    # prompt = generate_prompt(prompt_input, prompt_template)
    # fail_safe = get_fail_safe(act_game_object)
    # output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
...
def run_gpt_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, verbose=False):
    """
    アクションゲームオブジェクトとその説明を元にイベントトリプル（人物、動作、対象）を生成する関数
    """
...
def run_gpt_prompt_new_decomp_schedule(persona,
    ...
    """
    新しいスケジュールを生成するための関数。
    主なアクティビティの期間を短縮し、新しいアクティビティを挿入する。
    """
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 1000,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/new_decomp_schedule_v1.txt"
    prompt_input = create_prompt_input(persona,
                                       main_act_dur,
                                       truncated_act_dur,
                                       start_time_hour,
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved, test_input=None,
                                  verbose=False):
    """
    指定されたペルソナが他のペルソナと話すべきかどうかを決定する関数。
    """
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 20,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/decide_to_talk_v2.txt"
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved, test_input=None,
                                   verbose=False):
    """
    指定されたペルソナが他のペルソナに対してどのように反応するべきかを決定する関数。
    """
    def create_prompt_input(init_persona, target_persona, retrieved,
                            test_input=None):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 20,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/decide_to_react_v1.txt"
    prompt_input = create_prompt_input(persona, target_persona, retrieved,
                                       test_input)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 1000,
                 ...
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/create_conversation_v2.txt"
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/summarize_conversation_v1.txt"  ########
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/summarize_conversation_v1.txt"
    # prompt_input = create_prompt_input(conversation, test_input)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_extract_keywords(persona, description, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/get_keywords_v1.txt"
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 40,
                 "temperature": 0.7, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/keyword_to_thoughts_v1.txt"
...
def run_gpt_prompt_convo_to_thoughts(persona,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 40,
                 "temperature": 0.7, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/convo_to_thoughts_v1.txt"
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/poignancy_event_v1.txt"  ########
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/poignancy_event_v1.txt"
    # prompt_input = create_prompt_input(persona, event_description)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/poignancy_thought_v1.txt"  ########
    prompt_input = create_prompt_input(persona, event_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "5"  ########
    special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10."  ########
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/poignancy_thought_v1.txt"
    # prompt_input = create_prompt_input(persona, event_description)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/poignancy_chat_v1.txt"  ########
    prompt_input = create_prompt_input(persona, event_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "5"  ########
    special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10."  ########
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/poignancy_chat_v1.txt"
    # prompt_input = create_prompt_input(persona, event_description)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_focal_pt_v1.txt"  ########
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 150,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/generate_focal_pt_v1.txt"
    prompt_input = create_prompt_input(persona, statements, n)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 150,
                 "temperature": 0.5, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/insight_and_evidence_v1.txt"
    prompt_input = create_prompt_input(persona, statements, n)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_agent_chat_summarize_ideas(persona, target_persona, statements, curr_context, test_input=None,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/summarize_chat_ideas_v1.txt"  ########
    ...
    #              "temperature": 0.5, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/summarize_chat_ideas_v1.txt"
    # prompt_input = create_prompt_input(persona, target_persona, statements, curr_context)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_agent_chat_summarize_relationship(persona, target_persona, statements, test_input=None,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/summarize_chat_relationship_v2.txt"  ########
    ...
    #              "temperature": 0.5, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/summarize_chat_relationship_v1.txt"
    # prompt_input = create_prompt_input(persona, target_persona, statements)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/agent_chat_v1.txt"  ########
    ...
    #              "temperature": 0.7, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/agent_chat_v1.txt"
...
def run_gpt_prompt_summarize_ideas(persona, statements, question, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/summarize_ideas_v1.txt"  ########
    ...
    #              "temperature": 0.5, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v2/summarize_ideas_v1.txt"
    # prompt_input = create_prompt_input(persona, statements, question)
    # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None,
    ...
    #              "temperature": 0, "top_p": 1, "stream": False,
    #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    # prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_next_convo_line_v1.txt" ########
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 250,
                 "temperature": 1, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/generate_next_convo_line_v1.txt"
    prompt_input = create_prompt_input(persona, interlocutor_desc, prev_convo, retrieved_summary)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_generate_whisper_inner_thought(persona, whisper, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/whisper_inner_thought_v1.txt"
...
def run_gpt_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/planning_thought_on_convo_v1.txt"
...
def run_gpt_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False):
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/memo_on_convo_v1.txt"  ########
    prompt_input = create_prompt_input(persona, all_utt)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v2/memo_on_convo_v1.txt"
    prompt_input = create_prompt_input(persona, all_utt)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False):
    ...
    print("11")
    prompt_template = "backend/persona/prompt_template/safety/anthromorphosization_v1.txt"
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat,
    ...
    print("11")
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/iterative_convo_v1.txt"
...
#                          以下オリジナルの実装                                 #
##############################################################################
def run_gpt_prompt_act_obj_innate_desc(act_game_object, act_desp, persona, verbose=False):
    """アクションゲームオブジェクトとその説明を元にアクションを生成する関数"""
    def create_prompt_input(act_game_object, act_desp, persona):
        # プロンプトにペルソナの性格を追加
        prompt_input = [act_game_object,
    ...
    print("DEBUG: 個性に合わせた動作をします")  ########
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_obj_innate_event_v1.txt"  ########
    prompt_input = create_prompt_input(act_game_object, act_desp, persona)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "being fixed"  ########
    special_instruction = "The output should ONLY contain the phrase that should go in <fill in>."  ########
...
def run_gpt_prompt_update_obj_state(persona, act_game_object, current_state, inserted_act, new_address_, verbose=False):
    """
    指定された行動がオブジェクトの状態に与える影響を取得し、その結果を保存する関数
    """
    def create_prompt_input(persona, act_game_object, inserted_act):
        prompt_input = [
            act_game_object,
            persona.name,
            inserted_act,
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_obj_status_v1.txt"
    prompt_input = create_prompt_input(persona, act_game_object, inserted_act)
    prompt = generate_prompt(prompt_input, prompt_template)
    fail_safe = get_fail_safe(act_game_object)
...
def run_gpt_prompt_get_action_duration(persona, act_game_object, act_desp, verbose=False):
    """
    アクションの説明からそのアクションにかかる時間を取得する関数。
    GPT-3.5-turbo-instructモデルを使用します。
    :param persona: ペルソナクラスのインスタンス
    :param act_game_object: ゲームオブジェクトのインスタンス
    :param act_desp: アクションの説明文字列
    :param verbose: 詳細情報を表示するかどうか(オプション)
    :return: アクションにかかる時間(分)
    """
    def create_prompt_input(persona, act_game_object, act_desp):
        """アクションの説明とペルソナ情報からプロンプトを生成"""
        prompt_input = [
            act_game_object,
            persona.name,
            act_desp
    ...
    def __func_clean_up(gpt_response, prompt=""):
        """GPTのレスポンスを処理してアクションの所要時間を整数として返す"""
        cr = gpt_response.strip()
        if cr[-1] == ".": cr = cr[:-1]
        return int(cr.split()[0])
    ...
    def get_fail_safe():
        return "5 minutes"  # デフォルトのアクション時間

    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 10,
                 "temperature": 0.5, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}

    prompt_template = "backend/persona/prompt_template/v3_ChatGPT/generate_action_duration_v1.txt"
    prompt_input = create_prompt_input(persona, act_game_object, act_desp)
    prompt = generate_prompt(prompt_input, prompt_template)
    fail_safe = get_fail_safe()
...
def run_gpt_prompt_action_game_object_v2(game_object, act_desp, object_status,
                                      persona,
                                      maze,
                                      arena_address,
                                      test_input=None,
                                      verbose=False):
    """
    影響を及ぼすゲームオブジェクトとアクションの説明からペルソナの移動先オブジェクトの説明(性格もいる？)
    """
    ...
    def create_prompt_input(act_desp,
        ...
        prompt_input += [persona.name]
        prompt_input += [action_description]
        prompt_input += [game_object]
        prompt_input += [object_status]
        prompt_input += [persona
                         .s_mem.get_str_accessible_arena_game_objects(arena_address)]
        return prompt_input
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v1/action_object_v3.txt"
    ...
    x = [i.strip() for i in persona.s_mem.get_str_accessible_arena_game_objects(arena_address).split(",")]
    if output not in x:
        output = random.choice(x)
...
def run_gpt_prompt_action_sector_v2(act_game_object, inserted_act, current_state, persona, maze, new_address,
                                 test_input=None,
                                 verbose=False):
    def create_prompt_input(inserted_act, persona, maze, new_address, act_game_object, current_state, test_input=None):
        act_world = f"{maze.access_tile(persona.scratch.curr_tile)['world']}"

        prompt_input = []

        prompt_input += [persona.scratch.get_str_name()]  # 0: ペルソナの名前
        prompt_input += [persona.scratch.living_area.split(":")[1]]  # 1: 住居
        x = f"{act_world}:{persona.scratch.living_area.split(':')[1]}"
        prompt_input += [persona.s_mem.get_str_accessible_sector_arenas(x)]  # 2: 住居の設備

        prompt_input += [persona.scratch.get_str_name()]  # 3: ペルソナの参照
        prompt_input += [f"{maze.access_tile(persona.scratch.curr_tile)['sector']}"]  # 4: 現在の位置
        x = f"{act_world}:{maze.access_tile(persona.scratch.curr_tile)['sector']}"
        prompt_input += [persona.s_mem.get_str_accessible_sector_arenas(x)]  # 5: 現在の位置にある設備

        if persona.scratch.get_str_daily_plan_req() != "":
            prompt_input += [f"\n{persona.scratch.get_str_daily_plan_req()}"]  # 6: 追加の文脈情報
        else:
            prompt_input += [""]
        ...
        prompt_input += [accessible_sector_str]  # 7: エリアオプション

        prompt_input += [persona.scratch.get_str_name()]  # 8: アクションを行うペルソナの参照
        prompt_input += [inserted_act]  # 9: 現在の活動

        prompt_input += [inserted_act]  # 10: 現在のタスク

        prompt_input += [persona.scratch.get_str_name()]  # 11: 決定をするペルソナの参照
        prompt_input += [act_game_object]  # 12: ゲームオブジェクト
        prompt_input += [current_state]  # 13: ゲームオブジェクトの現在の状態
        prompt_input += [new_address]  # 14: ゲームオブジェクトのフルアドレス
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v1/action_location_sector_v3.txt"
    prompt_input = create_prompt_input(inserted_act, persona, maze, new_address, act_game_object, current_state)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_action_arena_v2(act_game_object, inserted_act, current_state, persona, maze, act_world, act_sector, new_address,
                                test_input=None,
                                verbose=False):
    def create_prompt_input(persona, act_game_object, inserted_act, current_state, maze, act_world, act_sector,
                            new_address):
        prompt_input = []
        # ペルソナの名前
        prompt_input += [persona.scratch.get_str_name()]  # 0
        # 行く予定の場所
        prompt_input += [act_sector]  # 1
        # 目的地のエリア
        x = f"{act_world}:{act_sector}"
        accessible_arena_str = persona.s_mem.get_str_accessible_sector_arenas(x)
        curr = accessible_arena_str.split(", ")
        ...
        prompt_input += [accessible_arena_str]  # 2
        # ペルソナの参照
        prompt_input += [persona.scratch.get_str_name()]  # 3
        # 現在のタスク
        prompt_input += [inserted_act]  # 4
        # 決定をするペルソナの参照
        prompt_input += [persona.scratch.get_str_name()]  # 5
        # ゲームオブジェクト
        prompt_input += [act_game_object]  # 6
        # ゲームオブジェクトの現在の状態
        prompt_input += [current_state]  # 7
        # ゲームオブジェクトのフルアドレス
        prompt_input += [new_address]  # 8
    ...
    gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15,
                 "temperature": 0, "top_p": 1, "stream": False,
                 "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = "backend/persona/prompt_template/v1/action_location_object_v2.txt"
    prompt_input = create_prompt_input(persona, act_game_object, inserted_act, current_state, maze, act_world, act_sector, new_address)
    prompt = generate_prompt(prompt_input, prompt_template)
